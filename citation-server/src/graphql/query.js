...
export async function buildObjects() {
	...
	const buildField = field => {
		...
		switch (fieldType) {
			...
			case 'map':
				return {type: new GraphQLList(KeyValuePair), resolve: root => readMap(root[field.name])};
			case 'json':
	...
	const ObjectTypes = fromPairs(model.map(structure => {
		return [structure.name, new GraphQLObjectType({
			...
			fields: () => ({
				...
				...fromPairs(structure.fields.map(field => [
					field.name,
					buildField(field)
				]))
			})
...
export async function buildQuery(ObjectTypes) {
	const query = new GraphQLObjectType({
		name: 'Query',
		fields: () => fromPairs(map(ObjectTypes, (value, key) => [
			key, {
